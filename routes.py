    ...
    try:
        ...
        for day in range(1, days_in_month + 1):
            ...
            question = questions.get(
                day, f"What was most meaningful to you today?")
...
@login_required
def chat_links():
    user_links = ChatLink.query.filter_by(user_id=current_user.id).order_by(
        ChatLink.created_at.desc()).all()
    return render_template('chat_links.html',
                           chat_links=user_links,
                           now=datetime.now())
...
def add_chat_link():
    ...
    if not url.startswith('https://chatgpt.com/share/'):
        flash(
            'Invalid ChatGPT share URL. URL must start with https://chatgpt.com/share/',
            'danger')
        return redirect(url_for('chat_links'))
